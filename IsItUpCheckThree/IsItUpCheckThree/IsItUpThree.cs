...
public class IsItUpThree
{
    ...
    private List<QueryResponse> queryList = new();
    ...
    [FunctionName("IsItUpThree")]
    public async Task<IActionResult> Run(
        [HttpTrigger(AuthorizationLevel.Function, "get", "post", Route = null)] HttpRequestMessage req,
        ILogger log, CancellationToken cancellationToken)
    {
        log.LogInformation("IsItUpCheckThree function processed a request from CNC.");
        ...
        try
        {
            queryList = await GetServers(serverList, log, cancellationToken).ConfigureAwait(false);
        }
    ...
    private async Task<List<QueryResponse>> GetServers(IEnumerable<string> list, ILogger log, CancellationToken cancellationToken)
    {
        ...
        //Timer for total response time for server polling
        ...
        foreach (var server in list)
        {
            ...
            //Browser headers to look like chrome to the WWW server
            ...
            _httpClient.DefaultRequestHeaders.Add("Keep-Alive", "300");
            ...
            try
            {
                ...
                catch (HttpRequestException e)
                {
                    log.LogInformation("Error in HTTP Request with the following information " + e.Message + " " + e.Data +
                                       " " + e.StatusCode.GetValueOrDefault() + " " + e.InnerException?.Message + " " + e.InnerException?.InnerException?.Message);
            ...
            finally
            {
                ...
                if (!IsNullOrEmpty(server))
                {
                    var item = new QueryResponse()
                    {
                        ...
                        responseServerResponse3 = StatusCode,
                        responseServerTime3 = totalTime,
                        responseServerCert3 = !IsNullOrEmpty(_expiryDate) ? _expiryDate : "Unknown",
                        responseServerNote3 = !IsNullOrEmpty(note) ? note : "No problems reported!"
                    };

                    queryList.Add(item);

                    log.LogInformation($"Queried {server} with Total Elapsed Time of {totalTime} and result was {StatusCode}.");
