...
public class IsItUpTwo
{
    ...
    private List<QueryResponse> queryResponseList = new();
    ...
    [FunctionName("IsItUpTwo")]
    public async Task<IActionResult> Run(
        [HttpTrigger(AuthorizationLevel.Function, "get", "post", Route = null)] HttpRequestMessage req,
        ILogger log, CancellationToken cancellationToken)
    {
        log.LogInformation("IsItUpCheckTwo function processed a request from CNC.");
        ...
        try
        {
            queryResponseList = await GetServers(serverList, log, cancellationToken).ConfigureAwait(false);
        }
        ...
        return new JsonResult(queryResponseList);
    }
    ...
    private async Task<List<QueryResponse>> GetServers(IEnumerable<string> list, ILogger log, CancellationToken cancellationToken)
    {
        ...
        //Timer for measure the polling total time per server
        ...
        foreach (var server in list)
        {
            ...
            //Headers to present the function as a browser
            ...
            finally
            {
                ...
                if (!IsNullOrEmpty(server))
                {
                    var item = new QueryResponse()
                    {
                        ...
                        responseServerResponse2 = StatusCode,
                        responseServerTime2 = totalTime,
                        responseServerCert2 = !IsNullOrEmpty(_expiryDate) ? _expiryDate : "Unknown",
                        responseServerNote2 = !IsNullOrEmpty(note) ? note : "No problems reported!"
                    };

                    queryResponseList.Add(item);

                    log.LogInformation($"Queried {server} and result was {StatusCode} and Response Time was {totalTime} ms.");
